...
BEGIN
    BEGIN
        -- Handle DELETE (Soft-Delete or Fallback to Sheet for Merged Assets)
        IF TG_OP = 'DELETE' THEN
            UPDATE public.core_vehicle_onboarding
            SET 
                source_system = CASE WHEN sheet_vehicle_id IS NOT NULL THEN 'GOOGLE_SHEET' ELSE source_system END,
                portal_vehicle_id = NULL,
                is_deleted = CASE WHEN sheet_vehicle_id IS NOT NULL THEN FALSE ELSE TRUE END,
                deleted_at = CASE WHEN sheet_vehicle_id IS NOT NULL THEN NULL ELSE (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Kolkata') END,
        ...
        ELSIF TG_OP IN ('INSERT', 'UPDATE') THEN
            ...
            IF v_clean_plate IS NOT NULL THEN
                -- Parse Dates
        ...
        END IF;
    EXCEPTION WHEN OTHERS THEN
        -- Fail-safe decoupling: Guarantees that source table july_vehicle_onboarding operations NEVER fail
        RAISE WARNING 'core_vehicle_onboarding sync warning from portal: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
            ...
            IF v_clean_plate IS NOT NULL THEN
                ...
                IF v_existing_id IS NOT NULL THEN
                    ...
                    SET 
                        ...
                        -- Non-conflicting Operational & Financial Fields: New non-blank sheet values take effect
                        ownership = COALESCE(NULLIF(NEW.ownership, ''), ownership),
                        financier = COALESCE(NULLIF(NEW.financier, ''), financier),
                        ageing = COALESCE(NULLIF(NEW.ageing, ''), ageing),
                        vehicle_status = COALESCE(NULLIF(NEW.vehicle_status, ''), vehicle_status, 'ACTIVE'),
                        delivery_date = COALESCE(NEW.delivery_date, delivery_date),
                        payment_date = COALESCE(payment_date, NEW.payment_date),
                        gps_status = COALESCE(NULLIF(NEW.gps, ''), gps_status),
                        pdi_status = COALESCE(NULLIF(NEW.pdi_status, ''), pdi_status),
                        platform = COALESCE(NULLIF(NEW.platform, ''), platform),
                        pdi_timestamp = COALESCE((NEW.pdi_timestamp AT TIME ZONE 'Asia/Kolkata')::TIMESTAMP, pdi_timestamp),
                        pdi_email_address = COALESCE(NULLIF(NEW.pdi_email_address, ''), pdi_email_address),
                        pdi_city = COALESCE(NULLIF(NEW.pdi_city, ''), pdi_city),
                        pdi_reg_no = COALESCE(NULLIF(NEW.pdi_reg_no, ''), pdi_reg_no),
                        mds_timestamp = COALESCE((NEW.mds_timestamp AT TIME ZONE 'Asia/Kolkata')::TIMESTAMP, mds_timestamp),
                        mds_email_address = COALESCE(NULLIF(NEW.mds_email_address, ''), mds_email_address),
                        mds_vehicle_number = COALESCE(NULLIF(NEW.mds_vehicle_number, ''), mds_vehicle_number),
                        permit_document = COALESCE(NULLIF(NEW.permit, ''), permit_document),
                        fitness_document = COALESCE(NULLIF(NEW.fitness, ''), fitness_document),
                        pollution_document = COALESCE(NULLIF(NEW.pollution, ''), pollution_document),
                        insurance_endorsement = COALESCE(NULLIF(NEW.insurance_endorsement, ''), insurance_endorsement),
                        invoice_copy = COALESCE(NULLIF(NEW.invoice_copy, ''), invoice_copy),
                        key_photo_url = COALESCE(NULLIF(NEW.key_photo_url, ''), key_photo_url),
                        rh_fr_tyre_brand_sl_no = COALESCE(NULLIF(NEW.rh_fr_tyre_brand_sl_no, ''), rh_fr_tyre_brand_sl_no),
                        lh_fr_tyre_brand_sl_no = COALESCE(NULLIF(NEW.lh_fr_tyre_brand_sl_no, ''), lh_fr_tyre_brand_sl_no),
                        rh_rear_tyre_brand_sl_no = COALESCE(NULLIF(NEW.rh_rear_tyre_brand_sl_no, ''), rh_rear_tyre_brand_sl_no),
                        lh_rear_tyre_brand_sl_no = COALESCE(NULLIF(NEW.lh_rear_tyre_brand_sl_no, ''), lh_rear_tyre_brand_sl_no),
                        spare_wheel_brand_sl_no = COALESCE(NULLIF(NEW.spare_wheel_brand_sl_no, ''), spare_wheel_brand_sl_no),
                        battery_sl_no = COALESCE(NULLIF(NEW.battery_sl_no, ''), battery_sl_no),
                        comments = COALESCE(NULLIF(NEW.comments, ''), comments),
                        ...
                        registration_date = CASE WHEN v_existing_portal_id IS NULL THEN NEW.registration_date ELSE registration_date END,
                        rto_tax_validity = CASE WHEN v_existing_portal_id IS NULL THEN NEW.rto_tax_validity ELSE rto_tax_validity END,
                        permit_validity = CASE WHEN v_existing_portal_id IS NULL THEN NEW.permit_validity ELSE permit_validity END,
                        fitness_validity = CASE WHEN v_existing_portal_id IS NULL THEN NEW.fitness_validity ELSE fitness_validity END,
                        pollution_validity = CASE WHEN v_existing_portal_id IS NULL THEN NEW.pollution_validity ELSE pollution_validity END,
                        insurance_validity = CASE WHEN v_existing_portal_id IS NULL THEN NEW.insurance_validity ELSE insurance_validity END,
                        ...
                        cng_installation_date = CASE WHEN v_existing_portal_id IS NULL THEN NEW.cng_installation_date ELSE cng_installation_date END,
                ...
                ELSE
                    ...
                    INSERT INTO public.core_vehicle_onboarding (
                        ...
                        lh_rear_tyre_brand_sl_no, spare_wheel_brand_sl_no, LEFT(NEW.battery_sl_no, 100),
        ...
        END IF;
    EXCEPTION WHEN OTHERS THEN
        -- Fail-safe decoupling: Guarantees that source table sheet_vehicle_onboarding operations NEVER fail
        RAISE WARNING 'core_vehicle_onboarding sync warning from sheet: %', SQLERRM;
    END;
