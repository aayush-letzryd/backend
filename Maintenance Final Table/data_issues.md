...
| **CORE-M-17** | All Sources | Architecture & Safety | Real-time trigger functions on raw source tables lacked unhandled exception shielding, exposing source operations to transaction aborts | Wrapped all 3 trigger functions (`fn_sync_core_maintenance_from_portal_in`, `fn_sync_core_maintenance_from_portal_out`, `fn_sync_core_maintenance_from_sheet`) with `EXCEPTION WHEN OTHERS THEN RAISE WARNING ... RETURN COALESCE(NEW, OLD);` blocks, guaranteeing 100% isolation so source table operations can never be blocked, aborted, or rolled back. | `CRITICAL` |
