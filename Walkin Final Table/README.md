...
If the Web Portal or Google Sheet introduces a new column in the future (for example, driver_experience or bank_account_number):
...
2. **Step-by-Step Guide to Add the New Column to core_walkin**:
   - **Step 1**: Add the column to core_walkin:
     ...
     ```
   - **Step 2**: Add the column mapping to the corresponding trigger function (e.g. fn_sync_core_walkin_from_portal_new):
