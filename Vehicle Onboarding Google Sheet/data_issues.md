...
| **2.9** | Year 1899 Epoch Date Corruption | 50 dates in `registration_date`, `pollution_validity`, and `insurance_validity` stored as `1899-12-30` from blank Excel cells. | Clamped dates `< 1950-01-01` to `NULL` in PostgreSQL and added `< 1950` sanity bounds in Apps Script parser. |
| **2.10** | Hourly Sequence Burning (48,000+ IDs consumed) | `INSERT ... ON CONFLICT (registration_no) DO UPDATE` burned 1,643 IDs every hourly run. | Re-engineered `UPSERT_SQL` into Zero-Burn CTE syntax (`WITH incoming AS ... upd AS UPDATE ... INSERT WHERE NOT EXISTS`). Renumbered live IDs to 1..1643 and reset sequence to 1643. |
| **2.11** | Formula Error Strings (`#N/A`, `#REF!`) | Unresolved Google Sheets formulas stored as literal string text in database. | Updated `cleanStr()` in Apps Script to convert `#N/A`, `#REF!`, `#VALUE!`, `NA`, and `-` into `NULL`. |
| **2.12** | Chassis Number Length False Positives | Valid commercial EVs (Tata Tigor/Ace EV with 18-19 chars) incorrectly flagged with `chassis_review_flag = TRUE`. | Removed 17-character length restriction. Set `chassis_review_flag = FALSE` in PostgreSQL and updated Apps Script. |
| **2.13** | Plaintext Credentials in Repository | Public IP and database password committed directly in code file. | Sanitized `getDbConfig()` and `setupScriptProperties()` to use placeholder values in GitHub repository. |
